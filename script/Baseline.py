...
ToView = [
         {'203M 205':'http://172.16.42.76:8819/overview/17'},
         {'20516 301C 206 205C 509' :'http://172.16.42.76:8819/overview/14'}
         ]
...
class QB(object):
    ...
    def add_QB_task(self):
        ...
        if find:
            view_value=find[0]
        view_tag   = find_data(u'<span class="label">视图</span>.*<select name="(?P<tag>.*?)" .*</select>.*<span class="label">标签', data, 'view_tag')
        label_tag  = find_data(u'<span class="label">标签</span>.*<textarea class=.* name="(?P<tag>.*?)" id=.*</textarea>', data, 'label_tag')
        find  = find_data(u'<span class="label">是否打基线</span>.*<option.*value="(?P<value>.*?)">yes</option>', data, 'Yes_tag')
        if find:
            yes_value=find[0]
        yes_tag  = find_data(u'<span class="label">是否打基线</span>.*<select name="(?P<tag>.*?)" id=.*</select>', data, 'Yes_tag')   
        ...
        url_data = {view_tag : view_value, label_tag : label, yes_tag : yes_value, user_tag : login_name, passwd_tag : login_passwd}
